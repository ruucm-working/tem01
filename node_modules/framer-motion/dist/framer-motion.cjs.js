...
var MountRef = React.memo(React.forwardRef(MountRefComponent));
...
function useValueAnimationControls(config, props, subscribeToParentControls, parentContext) {
    ...
    React.useEffect(function () { return function () {
        ...
        var onAnimationComplete = props.onAnimationComplete, unmountProps = tslib_1.__rest(props, ["onAnimationComplete"]);
...
var createMotionComponent = function (_a) {
    ...
    function MotionComponent(props, externalRef) {
        ...
        return (React.createElement(React.Fragment, null,
            React.createElement(MountRef, { ref: ref, externalRef: externalRef, values: values, isStatic: isStatic }),
            functionality,
...
function MotionPlugins(_a) {
    ...
    var value = React.useRef(tslib_1.__assign({}, pluginContext)).current;
...
function usePanGesture(_a, ref) {
    ...
    var session = React.useRef(null);
    var lastMoveEvent = React.useRef(null);
    var lastMoveEventInfo = React.useRef(null);
    var transformPagePoint = React.useContext(MotionPluginContext).transformPagePoint;
    var pointerEventSubscription = React.useRef(null);
    ...
    var handlers = React.useRef({}).current;
    ...
    function getPanInfo(_a) {
        ...
        return {
            point: point,
            delta: exports.Point.subtract(point, lastDevicePoint(currentPoint)),
            offset: exports.Point.subtract(point, startDevicePoint(currentPoint)),
    ...
    function updatePoint() {
        if (!session.current ||
            ...
            heyListen.warning(false, "onPointerMove fired without pointer session");
            cancelPan();
            return;
        }
        ...
        var distancePastThreshold = popcorn.distance(info.offset, { x: 0, y: 0 }) >= 3;
        if (!panStarted && !distancePastThreshold)
            return;
        var point = info.point;
        var timestamp = sync.getFrameData().timestamp;
        session.current.pointHistory.push(tslib_1.__assign({}, point, { timestamp: timestamp }));
        if (!panStarted) {
            handlers.onPanStart &&
    ...
    function onPointerMove(event, info) {
        ...
        sync__default.update(updatePoint, true);
    }
    function onPointerUp(event, info) {
        cancelPan();
        if (!session.current) {
            heyListen.warning(false, "onPointerUp fired without pointer session");
            return;
        }
        handlers.onPanEnd &&
    ...
    function onPointerDown(event, info) {
        ...
        session.current = {
            target: event.target,
            pointHistory: [tslib_1.__assign({}, point, { timestamp: timestamp })],
        };
...
var Gestures = {
    ...
    Component: makeRenderlessComponent(function (_a) {
        var innerRef = _a.innerRef, props = tslib_1.__rest(_a, ["innerRef"]);
...
function useDrag(_a, ref, values, controls) {
    ...
    var point = React.useRef({}).current;
    // Track origin
    ...
    var openGlobalLock = React.useRef(null);
    var transformPagePoint = React.useContext(MotionPluginContext).transformPagePoint;
    ...
    var prevConstraintsBox = React.useRef({
        width: 0,
        height: 0,
        x: 0,
        y: 0,
    ...
    function convertPanToDrag(info) {
        return tslib_1.__assign({}, info, { point: {
    ...
    function animateDragEnd(velocity) {
        var momentumAnimations = bothAxis(function (axis) {
            ...
            return animationControls.start((_a = {},
                ...
                _a.transition = tslib_1.__assign({ type: "inertia", velocity: dragMomentum ? velocity[axis] : 0, bounceStiffness: bounceStiffness,
...
var Drag = {
    ...
    Component: makeRenderlessComponent(function (_a) {
        var innerRef = _a.innerRef, values = _a.values, controls = _a.controls, props = tslib_1.__rest(_a, ["innerRef", "values", "controls"]);
...
function resolveCSSVariables(values, ref, _a, transitionEnd) {
    ...
    if (transitionEnd) {
        transitionEnd = tslib_1.__assign({}, transitionEnd);
    }
...
function useLayoutSync() {
    return React.useLayoutEffect(flushAllJobs);
}
...
function useLayoutAnimation(ref, values, controls, layoutTransition, positionOnly) {
    ...
    React.useContext(SyncLayoutContext);
    var element = ref.current;
    useLayoutSync();
    ...
    layoutSync.render(function () {
        ...
        function makeTransition(layoutKey, transformKey, targetValue, visualOrigin) {
            ...
            var baseTransition = typeof transitionDefinition === "boolean"
                ? tslib_1.__assign({}, getDefaultLayoutTransition(positionOnly)) : transitionDefinition;
            ...
            transition[transformKey] = baseTransition[transformKey]
                ? tslib_1.__assign({}, baseTransition[transformKey]) : tslib_1.__assign({}, baseTransition);
...
function targetWithoutTransition(_a, mergeTransitionEnd) {
    ...
    return mergeTransitionEnd
        ? tslib_1.__assign({}, target, transitionEnd)
        : target;
}
/**
...
function useAnimateProp(targetAndTransition, controls, values, defaultTransition) {
    ...
    React.useEffect(function () {
        ...
        prevValues.current = tslib_1.__assign({}, prevValues.current, finalTarget);
        if (Object.keys(targetToAnimate).length) {
            controls.start(tslib_1.__assign({}, targetToAnimate, { transition: targetAndTransition.transition || defaultTransition, transitionEnd: targetAndTransition.transitionEnd }));
        }
    }, [targetAndTransition]);
}
        ...
        React.useEffect(function () {
            if (isExiting) {
                if (!isPlayingExitAnimation.current && exit) {
                    controls.setProps(tslib_1.__assign({}, props, { custom: custom !== undefined ? custom : props.custom }));
...
var buildSVGProps = function (values, style) {
    ...
    props.style = tslib_1.__assign({}, style, props.style);
    return props;
};
...
function createDomMotionConfig(Component) {
    ...
    return {
        renderComponent: function (ref, style, values, props, isStatic) {
            ...
            return React.createElement(Component, tslib_1.__assign({}, forwardedProps, { ref: ref }, staticVisualStyles));
        },
        /**
        ...
        loadFunctionalityComponents: function (ref, values, props, context, controls, inherit) {
            ...
            for (var i = 0; i < numFunctionalityComponents; i++) {
                ...
                if (shouldRender(props, context)) {
                    activeComponents.push(React.createElement(Component_1, tslib_1.__assign({ key: key }, props, { parentContext: context, values: values, controls: controls, innerRef: ref })));
                }
            }
...
function transform() {
    ...
    var interpolator = popcorn.interpolate(inputRange, outputRange, tslib_1.__assign({ mixer: getMixer(outputRange[0]) }, options));
...
function useSpring(source, config) {
    ...
    React.useMemo(function () {
        return value.attach(function (v, set) {
            ...
            activeSpringAnimation.current = popmotion.spring(tslib_1.__assign({ from: value.get(), to: v, velocity: value.getVelocity() }, config)).start(set);
            return value.get();
        });
...
var PresenceChild = function (_a) {
    ...
    context = tslib_1.__assign({}, context, { exitProps: exitProps || {} });
